		...
		(function($, document) {
			...
			function initAddress(datas) {
				$.each(datas.data, function(index, getdata) {
					...
					var needString = "";
					if(index == 0) {
						needString = '<div class="some_address selected_address" id="' + address_id + '" uesFlag="' + use_flag + '">' + receiver_area + detail_address + '<img src="img/choose_blue.png" class="choose_yes"/>'
	+'</div>';
					} else if(index > 0) {
						needString = '<div class="line_all"></div>' +
							'<div class="some_address selected_address" id="' + address_id + '" uesFlag="' + use_flag + '">' + receiver_area + detail_address + '<img src="img/choose_blue.png" class="choose_yes"/>'
							+'</div>';
					}
			...
			$(".selected_address").bind("click", function() {
				...
				$(this).find("selected").show().siblings(".choose_yes").hide();
				//获取地址id
			...
			$(".add_new_address").bind("click",function(){
				window.location.href="addOrEdit_address.jsp?jump_flag=1";
			});

 ...
 * @param callback 回调函数
 */
function asyncAjax(url, parameter, callback){
    loadingAnimate();
    var serverUrl = "/" + url;
    $.ajax({
        type: "post",
        ...
        cache: false,
            ...
            console.log("mytools.js, aysncAjax:[" + serverUrl + "], success:[" + JSON.stringify(res) + "]");
            ...
            console.error("mytools.js, aysncAjax:[" + serverUrl + "], error:[" + jqXHR.status + ":" + textStatus + ":" + errorThrown + "]");
        }
    });
}
...
function myAjax(urlPath_parameter, need_data) {
	...
	var serverUrl = "/" + urlPath_parameter;
	var getdata_obj;
	$.ajax({
		type: "post"
		,url: serverUrl,
...
function loadSwiper(classOne, classTwo) {
	new Swiper('.' + classOne + '', {
		...
		autoplayDisableOnInteraction: false, //手动滑动之后还可以自动滑动
		//分页器
		pagination: '.' + classTwo + '',
		//centeedSlides: true,
	})
...
function popHtml(tips) {
	var html = '<div class="pop_up_first_bg">' +
		'<div class="pop_up_second_bg">' +
		'<div class="pop_up_content">' + tips + '</div>' +
		'</div>' +
		'</div>';
	$(html).appendTo("body");
	$(".pop_up_first_bg").show();
	setTimeout("hidePop()", 1500);
}

function hidePop() {
	$(".pop_up_first_bg").hide();
	$(".pop_up_first_bg").remove();
}

function loadingAnimate() {
	var animateString = '<div class="loading_animate">' +
		'<div class="loading_animate_imgcontainer">' +
		'<img class="loading_animate_img" src="img/loading_img.gif"/>' +
		'</div>' +
		'</div>';
